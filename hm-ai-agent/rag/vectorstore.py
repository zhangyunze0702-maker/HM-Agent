...
# --- 关键修改：在模块级别初始化，不要放进函数里 ---
_embeddings = HuggingFaceEmbeddings(
    model_name=EMBEDDING_MODEL_NAME,
    model_kwargs={'device': 'cpu'},  # 明确指定设备，防止 meta device 冲突
...
def get_shop_retriever(k=5):
    ...
    # --- 路 A: 向量检索 (Semantic Search) ---
    vector_retriever = vector_store.as_retriever(search_kwargs={"k": k})

    # --- 路 B: 关键词检索 (BM25 / Keyword Search) ---
    try:
        # 获取基础文档用于构建 BM25 索引
        ...
        if not all_docs:
            # 💡 替换 print 为 warning
            backend_log.warning("⚠️ Redis 中未获取到文档，BM25 索引构建跳过。")
...
# def get_shop_retriever(k=3):
#     """
#     返回一个标准的检索器 Retriever
#     """
#     vector_store = get_vector_store()
#
#     if vector_store is None:
#         raise ValueError("🚨 向量数据库初始化失败，请检查上方的错误日志！")
#
